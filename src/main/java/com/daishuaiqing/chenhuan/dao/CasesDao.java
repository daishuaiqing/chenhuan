...
public interface CasesDao extends JpaRepository<Cases, Long>,JpaSpecificationExecutor<Cases> {
    @Query(value = "SELECT * FROM `cases` WHERE `id`<?1 and is_deleted=0 ORDER BY `id` DESC LIMIT 1",nativeQuery = true)
    List<Cases> findLastCasesById(Long id);

    @Query(value = "SELECT * FROM `cases` WHERE `id`>?1 and is_deleted=0 ORDER BY `id` ASC LIMIT 1",nativeQuery = true)
